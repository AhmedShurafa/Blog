...
class NewCommentForPostOwnerNotify extends Notification implements ShouldQueue,ShouldBroadcast
{
    ...
    public function via($notifiable)
    {
        ...
        // if owner post he want receive email for not
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ->line('There is new comment from'.$this->comment->name
                                .'on your post'.$this->comment->post->title.'.')
            ->action('Go To Your Post', route('post.show',$this->comment->post->slug))
            ->line('Thank you for using BlogCMS System!');
    }
    ...
    public function toDatabase($notifiable)
    {
        return [
            ...
            'created_at'    => $this->comment->post->created_at->format('d M, Y h:i a'),
        ];
    }
